...
function scrollBox() {
	var nav = $("#header");
    var list = nav.find('.menu li a');
    var box = $('.section');
    var li = nav.find('.menu li');
    var title = $('head title');

    li.on("click", "a", function (event) {
		var id = jQuery(this).attr('href');
		if(id != '#' && id.length > 0){
	  	     var top = jQuery(id).offset().top - nav.innerHeight();
	        ...
	        if ($('.toogle-menu').is(":hidden")) {
	            ...
	            $('body,html').animate({scrollTop: top - nav.innerHeight()}, 1000, function () {
	                changeTitleHash(title, link.text(), link.attr('href').substr(1, link.attr('href').length - 1));
	                setTimeout(function () {
	                    $(window).on('scroll.check', all)
	                }, 200)
    ...
    function all() {
        ...
        time = setTimeout(function () {
            ...
            list.each(function () {
				...
				if (idn != '#' && idn.length > 0) {
					if ($(window).scrollTop() > $(idn).offset().top - 50 - nav.innerHeight()) {
	                    active = idn;
	                }
				}
            });
